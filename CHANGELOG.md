...
- The user authorized committing the accepted work, opening a PR, and deploying the CI-green revision in Docker on macmini-m1. This supersedes the earlier no-commit/no-deploy boundary for this task only; Docker is still forbidden on the development Mac. Repository visibility/CI selection is awaiting clarification after live inspection found a public repository using hosted runners, rather than the recalled private/self-hosted setup. No push, visibility change, or deployment has occurred at this preparation point.
...
- Current local evidence and batch authority: `/private/tmp/openppt-c-20260905-3ikfaee_/execution-contract.md`, `gate-0b-spec.md`, `gate-0c-spec.md`, and queued `c1-spec.md` through `c4-spec.md`. Independent dependency gate is `evidence/gate-0a-leader.md`; viewed public CJK outputs are under `evidence/leader-cjk/`; original HTTP repro is `evidence/leader-slow-pdf/red-22s.json`. Historical A+B evidence below remains unchanged.
